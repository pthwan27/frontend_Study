...
**중첩함수(함수 내에서 또 다른 함수를 선언하여 사용하는 함수)** 와 **콜백함수(setTimeout이라는 콜백함수)** 여기서 공통점은 **this**는 전역객체를 바인딩한다는 것입니다.
...
`someone.printName();` 암시적 바인딩에 의해 this가 someone으로 바인딩 되었다고 생각할 수도 있지만! 
...
클래스 내부는 엄격모드가 적용되고 일반적으로  this는 인스턴스 객체가 바인딩되고 중첩함수, 콜백함수로 일반 함수 선언, 호출 시 this에 undefined가 바인딩 됩니다. 
